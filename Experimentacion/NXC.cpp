...
// Función de partición para QuickSortNXC
int particionNXC(vector<Rectangle>& arr, int low, int high) {
...
// Función QuickSortNXC personalizada
void quickSortNXC(vector<Rectangle>& arr, int low, int high) {
    if (low < high) {
        int pi = particionNXC(arr, low, high);
        quickSortNXC(arr, low, pi - 1);
        quickSortNXC(arr, pi + 1, high);
    }
}
...
void generarNombreAleatorioNXC(char nombre[], int longitud) {
    random_device rd;
...
bool buildNearsX(vector<Rectangle> rectangles, char name[], int sizeMax){
    ...
    for (int i = 0; i < divisions.size(); i++){
        generarNombreAleatorioNXC(name,10);
        char ubq[23];
        strcpy(ubq, "./dNXC/");
        strcat(ubq, name);
        strcat(ubq, ".txt");
...
bool generateNearestX(vector<Rectangle>* rectangles, bool erase,int maxSize){
    if (erase){
        for (const auto& entry : fs::directory_iterator("./dNXC")) {
    ...
    quickSortNXC(rectanglesCopy,0,rectanglesCopy.size()-1);

    char name[23] = "./dNXC/raiz_NRX.txt";
    buildNearsX(rectanglesCopy, name, maxSize);
